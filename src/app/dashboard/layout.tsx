                                ...
                                {DASHBOARD_NAV_ITEMS.map((item) => {
                                    ...
                                    return (
                                        <Link
                                            key={item.href}
                                            href={item.href}
                                            className={`group relative flex items-center gap-3 px-4 py-4 rounded-xl transition-all duration-300 ${isActive ? "bg-white/[0.08] text-white" : "text-[var(--text-secondary)] hover:bg-white/[0.03] hover:text-white"
                                                }`}
                                        >
                                            {isActive && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-[var(--volt)] rounded-r-full shadow-[0_0_12px_var(--volt-glow)]" />}
                                            ...
                                            {isActive && <CaretRight size={16} weight="bold" className="ml-auto text-[var(--volt)]" />}
                                        </Link>
                                    );
                                })}
                            </nav>
                    ...
                    {DASHBOARD_NAV_ITEMS.map((item) => {
                        ...
                        return (
                            <Link
                                key={item.href}
                                href={item.href}
                                className={`group relative flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all duration-300 ${isActive ? "bg-white/[0.08] text-white" : "text-[var(--text-secondary)] hover:bg-white/[0.03] hover:text-white"
                                    }`}
                            >
                                {isActive && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-[var(--volt)] rounded-r-full shadow-[0_0_12px_var(--volt-glow)]" />}
                                <item.icon size={20} className={`transition-colors duration-300 ${isActive ? "text-[var(--volt)]" : "group-hover:text-white"}`} />
                                <span className="text-sm font-bold tracking-wide">{item.label}</span>
                                {isActive && <CaretRight size={14} weight="bold" className="ml-auto text-[var(--volt)]" />}
                            </Link>
                        );
                    })}
                </nav>
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.03] border border-[var(--border-subtle)]">
                                ...
                                <div className="min-w-0">
                                    <p className="text-sm font-bold text-white truncate">{formattedAddress}</p>
